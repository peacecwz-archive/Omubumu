...
class DbHandler {
    ...
    public function UyeEkle($AdiSoyadi,$Email,$KullaniciAdi,$Sifre){
        return DB::exec("INSERT INTO `uyeler` (`AdiSoyadi`, `KullaniciAdi`, `Sifre`, `Email`) VALUES (:adisoyadi, :kullaniciadi, :sifre, :email);",
    ...
    public function UyeGuncelle($UyeID,$AdiSoyadi,$Email,$Sifre){
        return DB::exec("UPDATE `uyeler` SET `AdiSoyadi`=:adisoyadi, `Sifre`=:sifre, `Email`=:email WHERE (`UyeID`=:UyeID);",
            array(':UyeID'=>$UyeID,':adisoyadi'=>$AdiSoyadi,':email'=>$Email,':sifre'=>$Sifre));
    }
    
    public function UyeResimGuncelle($UyeID,$ProfilResim){
        return DB::exec("UPDATE `uyeler` SET `ProfileImage`=:ProfilResim WHERE (`UyeID`=:UyeID);",
            array(':UyeID'=>$UyeID,':ProfilResim'=>$ProfilResim));
    }
    
    public function Giris($KullaniciAdi,$Sifre){
        $sonuc = DB::getRow("SELECT UyeID,KullaniciAdi,AdiSoyadi,Email,ProfileImage,Ban FROM `uyeler` WHERE `KullaniciAdi` =:username AND `Sifre`=:password",
            array(':username'=>$KullaniciAdi,':password'=>md5($Sifre)));
        return $sonuc;
    }
	
	public function KullaniciGetir($Mail){
        $sonuc = DB::getRow("SELECT * FROM `uyeler` WHERE `Email` =:Mail",
            array(':Mail'=>$Mail));
        return $sonuc;
    }
    
    public function KullaniciGetirByUyeID($UyeID){
        $sonuc = DB::getRow("SELECT UyeID,KullaniciAdi,AdiSoyadi,Email,ProfileImage FROM `uyeler` WHERE `UyeID` =:UyeID",
            array(':UyeID'=>$UyeID));
        return $sonuc;
    }
	
	    public function TamKullaniciGetirByUyeID($UyeID){
        $sonuc = DB::getRow("SELECT UyeID,Sifre,KullaniciAdi,AdiSoyadi,Email,ProfileImage FROM `uyeler` WHERE `UyeID` =:UyeID",
            array(':UyeID'=>$UyeID));
        return $sonuc;
    }
    
    public function SifirlamaKeyiGuncelle($Mail,$ResetKey){
        return DB::exec("UPDATE `uyeler` SET `SifirlamaKodu`=:SifirlamaKodu WHERE (`Email`=:Email);",
    ...
    public function Sorular(){
        return DB::get("SELECT sorular.SoruID,
                    ...
                    sorular.Baslik,
                    sorular.KategoriID,
                    DATE_FORMAT(sorular.SaveDate,'%d %M %Y') AS 'SaveDate',
    ...
    public function SoruEkle($UyeID,$Aciklama,$Baslik,$KategoriID,$Resim1,$Resim2){
        return DB::exec("INSERT INTO `sorular` (`UyeID`, `Resim1`, `Resim2`, `Aciklama`, `Baslik`, `KategoriID`) VALUES (:UyeID, :Resim1, :Resim2, :Aciklama,  :Baslik, :KategoriID);",
    ...
    public function Oyla($UyeID,$SoruID,$ResimNo){
        return DB::exec("INSERT INTO `oylar` (`UyeID`, `SoruID`, `ResimNo`) VALUES (:UyeID, :SoruID, :ResimNo);",
            array(':UyeID'=>$UyeID,':SoruID'=>$SoruID,':ResimNo'=>$ResimNo));
    }
    
    public function OyGuncelle($UyeID,$SoruID,$ResimNo){
        return DB::exec("UPDATE `oylar` SET `ResimNo`=:ResimNo WHERE (`UyeID`=:UyeID AND `SoruID`=:SoruID);",
            array(':UyeID'=>$UyeID,':SoruID'=>$SoruID,':ResimNo'=>$ResimNo));
    }
    ...
    public function YorumEkle($SoruID,$UyeID,$Yorum,$Resim){
        return DB::exec("INSERT INTO `yorumlar` (`UyeID`, `SoruID`, `Yorum`, `Resim`) VALUES ( :UyeID, :SoruID, :Yorum, :Resim );",
    ...
    public function Yorumlar($SoruID){
        return DB::get("SELECT
                    ...
                    yorumlar.Yorum,
                    yorumlar.Resim,
                    DATE_FORMAT(yorumlar.SaveDate,'%d %M %Y') AS 'SaveDate',
                    uyeler.AdiSoyadi,
                    uyeler.KullaniciAdi,
                    uyeler.ProfileImage
                    FROM
	...
	public function Log($IPAddress,$Useragent,$Mesaj){
		return DB::exec("INSERT INTO `omubumuapp`.`logs` (`UserAgent`, `IPAdress`, `Mesaj`) VALUES (:Useragent ,:IPAdres, :Mesaj);",
		array(':IPAdres'=>$IPAddress,':Useragent'=>$Useragent,':Mesaj'=>$Mesaj));
	}
}
?>